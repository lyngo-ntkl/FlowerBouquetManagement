...
namespace FlowerBouquetManagementSystemWebApp.Pages.User
{
    ...
    public class UserIndexFlowerBouquet : PageModel
    {
        ...
        public PaginatedList<FlowerBouquet> FlowerBouquets { get; set; } = default;
        //public SelectList Categories { get; set; }
        // search by name, id (for admin only), category, sort by date
        public async Task<IActionResult> OnGetFlowerBouquetAsync(string name, int? pageNumber, int? pageSize)
        {
            ...
            {
                flowerBouquets = flowerBouquets.Where(x => x.FlowerBouquetName.Contains(name.ToString())).ToList();
            }
            //if (categoryId != null)
            //{
        ...
        public void OnPostAddCart(int id, int? quantity)
        {
            ...
            } else
            {
                ...
                var flowerBouquetDto = _mapper.Map<FlowerBouquetDTO>(flowerBouquet);
                cart.AddToCart(new CartItem()
                {
                    Quantity = quantity ?? 1,
                    FlowerBouquetDTO = flowerBouquetDto
                });
            }
